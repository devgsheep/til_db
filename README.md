...
-- FK가 없는 경우에 데이터 오류가 발생하며 데이터 추가 안됨.

```sql
INSERT INTO 테이블명 (컬럼명, 컬럼명, ...) VALUES (값, 값, ...);
-- FK 에 없는 사용자 id를 넣는다면?
insert into posts (id, title, contents, authors_id) values (3, "제목2", "내용2", 1);
...
- 주의사항 : 기존 프로그래머들
  ...
  - `=`을 사용하여 같다고 한다. 즉, 비교한다.

```sql
UPDATE 테이블명 SET 컬럼명 = 값, 컬럼명 = 값 WHERE 조건;
UPDATE 테이블명 SET 컬럼명 = 값, 컬럼명 = 값 WHERE 필드이름 = 값;
-- 사용자 id 1의 email을 변경하기
...
- 여러 컬럼을 정렬함. (앞쪽 처리 후, 나머지 순으로 처리됨)
...
- 가장 많이 사용
- 주문시간, 글 작성시간 등
- `칼럼명 datetime default current_timestamp()`
